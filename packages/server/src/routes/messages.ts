...
app.post('/api/message', async (c) => {
    ...
    const { message, agent, sender, senderId, channel, files, messageId: clientMessageId, topicId } = body as {
        message?: string; agent?: string; sender?: string; senderId?: string;
        channel?: string; files?: string[]; messageId?: string; topicId?: string;
    };
